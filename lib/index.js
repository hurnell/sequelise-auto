...
var isOldVersion = 5 > parseInt(Sequelize.version, 10);
if(isOldVersion){
   _ = Sequelize.Utils._;
}
...
AutoSequelize.prototype.build = function(callback) {
  ...
  function processTables(__tables) {
    var tableNeedsMapping = 'mssql' === self.sequelize.options.dialect || ('mariadb' === self.sequelize.options.dialect && !isOldVersion)
    if ( tableNeedsMapping){
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      var upperStartName  = tableName.replace(/^(tbl)/,'') + 'Entity';
      var entityName =  upperStartName.charAt(0).toLowerCase() + upperStartName.substring(1);
